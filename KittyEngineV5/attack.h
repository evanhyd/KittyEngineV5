  ...
  static constexpr auto generateMagic() {
    ...
    for (Piece piece : {kBishop, kRook}) {
      ...
      for (uint32_t i = 0; i < kSquareSize; ++i) {
        ...
        magic.maxAttackNoEdge = generateSliderAttackReachable(piece, i, 0) & ~edge;
        ...
        for (Bitboard occupancy = magic.maxAttackNoEdge; ; occupancy = (occupancy - 1) & magic.maxAttackNoEdge) {
          magic.attackReachable[_pext_u64(occupancy, magic.maxAttackNoEdge)] = generateSliderAttackReachable(piece, i, occupancy);
          if (occupancy == 0) {
            break;
          }
        }
...
namespace internal {
  template <Piece>
  constexpr int kRelevantBitsInversed;

  template<>
  inline constexpr std::array<uint32_t, kSquareSize> kRelevantBitsInversed<kBishop> = {
  ...
  template<>
  inline constexpr std::array<uint32_t, kSquareSize> kRelevantBitsInversed<kRook> = {
  ...
  template <Piece>
  constexpr int kMagicNumTable;

  template<>
  inline constexpr std::array<Bitboard, kSquareSize> kMagicNumTable<kBishop> = {
      0x40040844404084ULL,
      0x2004208a004208ULL,
      0x10190041080202ULL,
  ...
  template<>
  inline constexpr std::array<Bitboard, kSquareSize> kMagicNumTable<kRook> = {
  ...
  template <Piece>
  constexpr int kAttackNoEdgeTable;

  template <>
  inline constexpr auto kAttackNoEdgeTable<kBishop> = []() {
    std::array<Bitboard, kSquareSize> table{};
    for (uint32_t i = 0; i < kSquareSize; ++i) {
      Bitboard edge = ((kRank1Mask | kRank8Mask) & ~kSquareToRankMaskTable[i]) | ((kFileAMask | kFileHMask) & ~kSquareToFileMaskTable[i]);
      table[i] = generateSliderAttackReachable(kBishop, i, 0) & ~edge;
    }
    return table;
  }();

  template <>
  inline constexpr auto kAttackNoEdgeTable<kRook> = []() {
    std::array<Bitboard, kSquareSize> table{};
    for (uint32_t i = 0; i < kSquareSize; ++i) {
      Bitboard edge = ((kRank1Mask | kRank8Mask) & ~kSquareToRankMaskTable[i]) | ((kFileAMask | kFileHMask) & ~kSquareToFileMaskTable[i]);
      table[i] = generateSliderAttackReachable(kRook, i, 0) & ~edge;
    }
    return table;
  }();
}
...
class SliderTable {
  static const std::array<std::array<Bitboard, 512>, kSquareSize> kBishopAttackTable;
  static const std::array<std::array<Bitboard, 4096>, kSquareSize> kRookAttackTable;

  template <Piece piece>
  static constexpr auto generateMagic() {
    auto table = []() {
      if constexpr (piece == kBishop) {
        return std::array<std::array<Bitboard, 512>, kSquareSize>{};
      } else {
        return std::array<std::array<Bitboard, 4096>, kSquareSize>{};
      }
    }();

    for (uint32_t i = 0; i < kSquareSize; ++i) {
      // Hash the occupancy into the magic index
      for (Bitboard occupancy = internal::kAttackNoEdgeTable<piece>[i]; ; occupancy = (occupancy - 1) & internal::kAttackNoEdgeTable<piece>[i]) {
        size_t key = (occupancy * internal::kMagicNumTable<piece>[i]) >> internal::kRelevantBitsInversed<piece>[i];
        table[i][key] = internal::generateSliderAttackReachable(piece, i, occupancy);
        if (occupancy == 0) {
          break;
        }
      }
  ...
  template <Piece piece>
  static constexpr size_t getAttackTableKey(uint32_t square, Bitboard occupancy) {
    const Bitboard attackNoEdge = internal::template kAttackNoEdgeTable<piece>[square];
    const Bitboard magicNum = internal::template kMagicNumTable<piece>[square];
    const Bitboard releventBitsCount = internal::template kRelevantBitsInversed<piece>[square];
    return ((occupancy & attackNoEdge) * magicNum) >> releventBitsCount;
  }
...
public:
  ...
  static Bitboard getAttack(uint32_t square, Bitboard occupancy) {
    if constexpr (piece == kBishop) {
      return kBishopAttackTable[square][getAttackTableKey<piece>(square, occupancy)];
    } else if constexpr (piece == kRook) {
      return kRookAttackTable[square][getAttackTableKey<piece>(square, occupancy)];
    } else if constexpr (piece == kQueen) {
      return kBishopAttackTable[square][getAttackTableKey<kBishop>(square, occupancy)] | kRookAttackTable[square][getAttackTableKey<kRook>(square, occupancy)];
    } else {
      static_assert(false, "invalid piece type");
    }
  }
...
inline const std::array<std::array<Bitboard, 512>, kSquareSize> SliderTable::kBishopAttackTable = SliderTable::generateMagic<kBishop>();
inline const std::array<std::array<Bitboard, 4096>, kSquareSize> SliderTable::kRookAttackTable = SliderTable::generateMagic<kRook>();
